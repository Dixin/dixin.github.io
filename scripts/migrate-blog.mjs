...
turndownService.addRule("preCodeBlock", {
	filter: (node) => node.nodeName === "PRE" && node.classList.contains("code"),
	replacement: (_content, node) => {
...
turndownService.addRule("codeCxSpMiddle", {
	filter: (node) =>
		node.nodeName === "P" && node.classList.contains("CodeCxSpMiddle"),
	replacement: (_content, node) => {
		// Check if this is the first in a sequence
		const prevSibling = node.previousElementSibling;
		const isFirst =
			!prevSibling || !prevSibling.classList.contains("CodeCxSpMiddle");

		// Check if this is the last in a sequence
		const nextSibling = node.nextElementSibling;
		const isLast =
			!nextSibling || !nextSibling.classList.contains("CodeCxSpMiddle");
		...
		if (isFirst) {
			...
			const codeLines = [lineText];
			let current = node.nextElementSibling;
			while (current?.classList.contains("CodeCxSpMiddle")) {
...
function detectLanguage(codeNode) {
	...
	if (classes.includes("javascript") || classes.includes("js"))
		return "javascript";
	if (classes.includes("typescript") || classes.includes("ts"))
		return "typescript";
...
function normalizeLinks(contentElement) {
	...
	links.forEach((link) => {
		...
		// Check if it's an absolute URL containing weblogs.asp.net/dixin
			...
			if (dixinIndex !== -1) {
				const afterDixin = href.substring(
					dixinIndex + "weblogs.asp.net/dixin/".length,
				);
				const lastSegment = afterDixin
					.split("/")[0]
					.split("?")[0]
					.split("#")[0];
				if (lastSegment && lastSegment.length > 0) {
...
async function fetchPostData(url) {
	...
	const elementsToRemove = content.querySelectorAll(
		"script, style, .comments, #comments, .share, .social, .related-posts, .navigation, .nav",
	);
